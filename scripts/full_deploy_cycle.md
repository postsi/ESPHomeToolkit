...
- **Self-update 403 (“can’t update itself”):** Supervisor intentionally returns **403** when an add-on calls **`POST /addons/{slug}/update`** for **its own** slug (see `supervisor/api/store.py`: `addons_addon_update`). That is **not** random token corruption — it happens whenever MCP tools run **inside EspToolkit** and try to update EspToolkit via the Supervisor API. **Fix (in-addon):** compare the requested slug to **`GET /addons/self/info`**, and for a match call Home Assistant **`hassio.addon_update`** via Core REST instead (needs **`homeassistant_api: true`**, which this add-on already sets). Older docs assumed a different MCP host; with tools embedded in EspToolkit, the Core path is required for self-update.
